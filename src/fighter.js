function doubleSliceWeapons(actor) {
    return actor.system.actions
        .filter( h => h.ready && (h.item?.isMelee || ((h.item?.isThrown || h.item?.isRanged) && actorFeat(actor, "dual-thrower")!=0)) && h.item?.isHeld && h.item?.hands === "1" && h.item?.handsHeld === 1 && !h.item?.system?.traits?.value?.includes("unarmed") );
};
